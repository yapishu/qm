...
The sandbox handoff is a substrate image pin plus a layer content hash. Fly and Docker with Sprites use `sandbox publish` to push an OCI image, resolve its immutable digest, and record it in the config. Docker with `sandbox.backend: "local"` uses `sandbox build`; for a single host the operator records the built image's bare `sha256:…` ID, while portable deployments tag and push it and record its registry digest. `up` loads either immutable reference into the deployment's isolated nested sandbox daemon. AWS with `sandbox.backend: "aws"` (or no sandbox block) uses `infra build-image` to package the guest agent as a Lambda MicroVM image and records its immutable image version and execution role; with `sandbox.backend: "sprites"`, `sandbox publish` pushes the layer image and records its digest pin in the durable deployment manifest, which `up`, `check --live`, and `rollback` resolve. Service task definitions and sandbox root filesystems use immutable pins, not mutable tags.
...
| Agent-computer image and credentials                                                       |    local image or Fly app/token |        Fly app and scoped token | Lambda MicroVM image/version and execution role |
